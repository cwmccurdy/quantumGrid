...
for j in range(0, fem_dvr.nbas):
    ...
    Cinitial[j] = np.sqrt(np.sqrt(2.0*alpha/np.pi))*np.exp(-alpha *
                                                           (fem_dvr.x_pts[j+1]-shift)**2)*np.sqrt(fem_dvr.w_pts[j+1])
...
for i_time_interval in range(0, n_intervals):
    ...
    Ctfinal = fem_dvr.Crank_Nicolson(t_start, t_finish, N_time_steps,
                                 Cinitial, pertubation.vectorized_V_Bernstein, Is_H_time_dependent=False)
