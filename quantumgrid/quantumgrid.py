...
class scatterGrid(object):
    ...
    def __init__(self, n_order, FEM_boundaries, Mass=1, Complex_scale=1, R0_scale=0.0):
        ...
        print("\nFrom FEM_DVR_build: building grid and KE with Gauss Lobatto quadrature of order ", n_order)
        #
        ...
        #
        for i_elem in range(0, N_elements):
            print("element ", i_elem + 1, " xmin = ",
                  FEM_boundaries[i_elem], " xmax = ", FEM_boundaries[i_elem + 1])
        #
        ...
        for i_elem in range(0, N_elements):
            ...
            for i in range(0, n_order):
                for j in range(0, n_order):
                    KE_temp[i + (i_elem * (n_order - 1)), j + (i_elem * (n_order - 1))] = \
                        KE_temp[i + (i_elem * (n_order - 1)), j +
                                (i_elem * (n_order - 1))] + KE_elem[i, j]
        ...
        for i in range(0, nbas):
            for j in range(0, nbas):
                KE_mat[i, j] = KE_temp[i + 1, j + 1] / \
                    np.sqrt(w_pts[i + 1] * w_pts[j + 1])
        # KE matrix complete, return full grid with endpoints and KE_mat which is nbas x nbas
        self.nbas = nbas
        self.x_pts = x_pts
        self.w_pts = w_pts
        self.KE_mat = KE_mat
    ...
    def Potential_Two_States(self, V_potential_1, V_potential_2, V_coupling, time):
        ...
        potential_1[jj, jj] = V_potential_1(
            x[jj + 1], time)  # potential for state 1
        potential_2[jj, jj] = V_potential_2(
            x[jj + 1], time)  # potential for state 2
    ...
    def psi_evaluate(self, x, coef_vector):
        ...
        if (i_elem < 0):
            print('x value ', x, ' is out of range ', FEM_boundaries[0],
                  ', ', FEM_boundaries[N_elements], ' in psi_evaluate()')
            exit()
        ...
        for j_fcn in range(index_left, index_left + N_order):
            ...
            for i in range(index_left, index_left + N_order):
                if (i != j_fcn):
                    funcval = funcval * \
                        (x - x_grid[i]) / (x_grid[j_fcn] - x_grid[i])
            sum_val = sum_val + Coef * funcval
        psi_value = sum_val
        return psi_value

    def Plot_Psi(self, Psi_coefficients, plot_title_string="Plot of FEM-DVR representation", N_plot_points=500, make_plot=True):
        #
        ...
        for i_elem in range(0, N_elements):
            dx = (FEM_boundaries[i_elem + 1] -
                  FEM_boundaries[i_elem])/N_pts_per_elem
            if(i_elem == N_elements - 1):
                dx = (FEM_boundaries[i_elem + 1] -
                      FEM_boundaries[i_elem])/(N_pts_per_elem-1)
        ...
        if (make_plot):
            ...
            plt.savefig('Plot_Output/' + plot_title_string +
                        '.pdf', transparent=False)
    ...
    def Plot_Psi_Two_States(self, Psi_coefficients1, Psi_coefficients2, plot_title_string="Plot of FEM-DVR representation", N_plot_points=500, make_plot=True):
        #
        ...
        N_order = self.n_order
        N_elements = len(FEM_boundaries) - 1
        dx = (FEM_boundaries[N_elements] -
              FEM_boundaries[0]) / (N_plot_points - 1)
        Psi1_Plot = []
        Psi2_Plot = []
        x_Plot = []
        ...
        if (make_plot):
            ...
            plt.plot(x_Plot, np.real(Psi2_Plot), '-r',
                     linestyle='--', label=string21)
            plt.plot(x_Plot, np.imag(Psi2_Plot), '-g',
                     linestyle='--', label=string22)
            plt.plot(x_Plot, np.abs(Psi1_Plot), '-k', label=string13)
            plt.plot(x_Plot, np.abs(Psi2_Plot), '-k',
                     linestyle='--', label=string23)
            plt.legend(loc="best")
            plt.xlabel(" x ", fontsize=14)
            plt.ylabel("psi", fontsize=14)
            #     limits if necessary
            ...
            plt.savefig('Plot_Output/' + plot_title_string +
                        '.pdf', transparent=False)
            plt.show()  # note plt.show() evidently clears everything for this plot
        return x_Plot, Psi1_Plot, Psi2_Plot  # returns the x, y coordinates for a graph

    def Crank_Nicolson(self, t_initial, t_final, N_times, Coefs_at_t_initial, potential, Is_H_time_dependent=True):
        #
        ...
        print("Start Crank Nicolson propagation from ", t_initial, " to ", t_final, " atu"
              "\n                              in steps of ", Deltat, " with ", N_times, " steps")
...
#
        if (Is_H_time_dependent):
            ...
            for itime in range(1, N_times + 1):
                ...
                potential_correction_M = 1.0 + \
                    (1j*Deltat/2.0)*(KE[i, i] +
                                     potential(x[i+1], time-Deltat/2.0))
                potential_correction_Mconj = 1.0 - \
                    (1j*Deltat/2.0)*(KE[i, i] +
                                     potential(x[i+1], time-Deltat/2.0))
    ...
    def Crank_Nicolson_Two_States(self, t_initial, t_final, N_times, Coefs_at_t_initial,
                                  V_potential_1, V_potential_2, V_coupling, Is_H_time_dependent=True):
        #
        #  Crank Nicolson Propagator for time-dependent or time-independent Hamiltonian
        #      Both implementations are unitary.  A time step is
        #  C_t = (1 + i H(t-Deltat/2) * Deltat/2)^-1 * (1 - i H(t-Deltat/2) * Deltat/2) C_t-Deltat
        #
        #  In this routine the wave function has two components propagating on coupled potential curves
        #  specified by V_potential_1, V_potential_2 and V_coupling
        #
        #  Time-dependent case can slow.  Building the Hamiltonian and M matrices dominates for
        #  small grids, but linear equation solve dominates time for large grids.
        #  This implementation allows both the diagonal and coupling potentials to be time-dependent
        #  and allows the coupling to be complex hermitian.
        #
        nbas = self.nbas
        t_interval = t_final - t_initial
        Deltat = t_interval / np.float(N_times)
        ...
        # build full H at time-Deltat/2, for step from t-Deltat to t
                ...
                potential_correction = 1.0
                np.fill_diagonal(M, M.diagonal() + potential_correction)
                np.fill_diagonal(Mconj, Mconj.diagonal() +
    ...
    def rescale_kinetic(self, Mass):
        # ===============================Kinetic Energy for this reduced mass ===================#
        # initialize KE_mat with the Kinetic energy matrix multiplied by 1 / reduced mass
        self.KE_mat = (1.0 / Mass) * self.KE_mat

    def hello_world(self):
        print("HelloWorld from scatterGrid class!")
